...
This repository contains production-pattern workflow automation projects built with n8n, demonstrating progressive complexity across data validation, sub-workflow orchestration, parallel data processing, analytical reporting pipelines, MLOps monitoring, and AI-powered document retrieval.
...
## Project 2: Automated Sales Report Pipeline
...
- Summarize node for team-level aggregation
...
The primary difficulty was in the analytical code layer — transforming raw row-level data into reduced, meaningful metrics. The core problem was understanding that each item flowing through the pipeline represents one record, and meaningful analysis requires either reducing multiple records into one or enriching each record with derived values before routing decisions can be made.

The join operation between sales data and targets required matching records by representative name across two datasets — a nested loop pattern that was unintuitive coming from a node-based drag-and-drop mental model.
...
## Project 3: ML Model Monitoring Pipeline

*In progress*

---

## Project 4: Company Docs RAG Chatbot
...
│   └── sales-report-pipeline.json
├── ML Model Monitoring Pipeline/
│   └── ml-model-monitoring.json
└── Company Docs RAG Chatbot/
    └── company-docs-rag-chatbot.json
```
